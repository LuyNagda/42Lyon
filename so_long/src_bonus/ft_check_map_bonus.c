...
/*   Updated: 2023/12/15 16:06:13 by lunagda          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_check_walls(t_data *data)
{
	...
	while (y < data->map.rows)
	{
		...
		{
			if ((y == 0 || y == data->map.rows - 1) \
			&& data->map.full[y][x] != '1')
			{
				ft_free_map(data);
				ft_error_msg("Invalid map. Not covered by walls.\n", data);
			}
			else if ((x == 0 || x == data->map.columns - 1) \
			&& data->map.full[y][x] != '1')
			{
				ft_free_map(data);
				ft_error_msg("Invalid map. Not covered by walls.\n", data);
	...
	while (data->map.full[y])
	{
		...
		while (data->map.full[y][x])
		{
			...
			if (data->map.full[y][x] == COINS)
				data->map.coins++;
			if (data->map.full[y][x] == TOXIC_RIVER)
				data->map.toxic_river++;
			x++;
		}
		y++;
	}
...
void	ft_map_is_rectangular(t_data *data)
{
	...
	while (i < data->map.rows)
	{
		...
		{
			ft_free_map(data);
			ft_error_msg("Invalid map. The map is \
...
void	ft_check_map(t_data *data)
{
	...
	while (y < data->map.rows)
	{
		...
		while (x < data->map.columns)
		{
			if (!ft_strchr("CEPT10", data->map.full[y][x]))
			{
				ft_free_map(data);
